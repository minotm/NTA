...
def create_parser():
    ...
    parser.add_argument('--aug_type', type=str, default='iterative',
                        help = 'Augmentation Options: random, iterative, codon_shuffle, codon_balance, online, online_balance \n note that codon_shuffle and codon_balance are iterative augmentation methdos')
...
print(f'Nucleotide Augmentation of GB1 Data Set with Augmentation Type = {args.aug_type}')
...
if 'random' in args.aug_type or 'online' in args.aug_type:
    for i in range(5):
        out_file_str_test = data_path + f'gb1_three_vs_rest_test_dna_{args.aug_type}_{i}.csv'
    ...
    out_file_str_test = data_path + f'gb1_three_vs_rest_test_dna_{args.aug_type}.csv'
    test_aug = nt_augmentation(input_seqs = test_full, args= args,
                                aug_factor = 1, is_val_set=True)
        ...
        for key in aug_dict:
            ...
            for i in range(len(aug_dict[key])):
                ...
                if 'random' in args.aug_type:
                    for k in range(5):
                        out_file_str_train = outpath + 'gb1_' + data_str + '_train_' + 'truncated_' + str(truncate_factor) + f'_dna_aug_{i+1}_{args.aug_type}_{k}.csv'
                    ...
                    out_file_str_train = outpath + 'gb1_' + data_str + '_train_' + 'truncated_' + str(truncate_factor) + f'_dna_aug_{i+1}_{args.aug_type}.csv'
                    out_file_str_val = outpath + 'gb1_' + data_str + '_val_' + 'truncated_' + str(truncate_factor) + f'_dna_aug_{i+1}_{args.aug_type}.csv'
                    train_aug = nt_augmentation(input_seqs = train,  args= args,
                                                aug_factor = aug_factor, is_val_set=False)
    ...
    for truncate_factor in truncate_factor_list:
        for aug_factor in aug_factor_list:
            ...
            if 'random' in args.aug_type or args.aug_type.startswith('online'):
                ...
                #actual_aug_type = args.aug_type
                #args.aug_type = 'random'
                
                for i in range(5):
                    out_file_str_train = outpath + 'gb1_' + data_str + '_train_' + 'truncated_' + str(truncate_factor) + f'_dna_aug_none_{args.aug_type}_{i}.csv'
                    out_file_str_val = outpath + 'gb1_' + data_str + '_val_' + 'truncated_' + str(truncate_factor) + f'_dna_aug_none_{args.aug_type}_{i}.csv'
                ...
                #args.aug_type = actual_aug_type
                
            else:               
                outpath = data_path
                out_file_str_train = outpath + 'gb1_' + data_str + '_train_' + 'truncated_' + str(truncate_factor) + f'_dna_aug_none_{args.aug_type}.csv'
                out_file_str_val = outpath + 'gb1_' + data_str + '_val_' + 'truncated_' + str(truncate_factor) + f'_dna_aug_none_{args.aug_type}.csv'
