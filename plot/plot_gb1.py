...
cnn = 'gb1_three_vs_rest_cnn2.csv'
transformer = 'gb1_three_vs_rest_t2.csv'
...
aug_factor_dict = {'1': '2', '2': '5', '3': '10', '4':'50', '5': '100'} #rename augmentation datasets to their true n_aug values5

aug_type_list = aug_type_list[1:]   
#aug_type_list = ['iterative']
...
for model in model_str_list:
    ...
    #drop all 'none' aug factor from dataframe. this includes all online augmentation and baselines
    ...
    for aug_type in aug_type_list_out:
        ...
        aa_data = full_data[(full_data['seq_type'] == 'aa') & (full_data['on_off'] == on_off) & (full_data['model'] == model)].copy()
    ...
    results.fig.legend(handles=results.legend.legendHandles[1:12], loc=7, frameon= False, title = 'Augmentation Factor ($n_{aug}$)')
    results.set(xscale = 'log')
    
    data_type = 'gb1'
    plt.subplots_adjust(wspace=0.1)
    results.set(ylim=(0.0, 0.9))
